...
export const generatePrompt = async (userGoal: string, targetAI: string): Promise<GeneratePromptResult> => {
    try {
...
export const startOrContinueInterview = async (interviewLog: InterviewLogItem[], targetAI: string): Promise<InterviewResult> => {
    try {
        ...
        return { type: 'error', content: { error: true, message: "The prompt chef got confused during the conversation.", suggestions: ["Try rephrasing your last answer.", "Consider restarting the session."] } };
    }
};
